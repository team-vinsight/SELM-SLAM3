    ...
    mbFinishRequested(false), mbFinished(true), mbStopped(true), mbStopRequested(false)
{
    ...
    // cout << endl << "B.B In Viewer.cc, Viewer::Viewer, pTracking->mState=" << pTracking->mState << endl;
    if(settings) {
        // cout << "B.B In Viewer::Viewer constructor. Pointer to settings object is not null ..." << endl; 
...
void Viewer::newParameterLoader(Settings *settings) {
    ...
    // cout << endl << "B.B In Viewer::newParameterLoader, H, W, ImageViewerScale, Viewpoint X, Y, Z, F:" << mImageHeight << ", " << mImageWidth << ", " << mImageViewerScale << ", " << mViewpointX << ", " << mViewpointY << ", " << mViewpointZ << ", " << mViewpointF << endl;
}
...
bool Viewer::ParseViewerParamFile(cv::FileStorage &fSettings) {
    ...
    // cout << endl << "B.B In Viewer::ParseViewerParamFile, b_miss_params=" << b_miss_params << endl;
...
void Viewer::Run() {
    ...
    // pangolin::CreateWindowAndBind("ORB-SLAM3: Map Viewer", 1024, 768); commented by B.B
    pangolin::CreateWindowAndBind("SELM-SLAM3: Map Viewer", 1024, 768);
    ...
    // B.B creates a control panel in the window, setting its position at the top with a height of 175 pixels.
    ...
    cout << endl << "B.B In Viewer::Run. Starting the Viewer..." << endl;
    while(1) {
        ...
        // B.B Displays the current frame
        cv::Mat toShow;
        cv::Mat im = mpFrameDrawer->DrawFrame(trackedImageScale); // B.B This line provide the image to be shown in the Viewer

        // cout << endl << "B.B In Viewer::Run, before showing frame, the value of both is: " << both << endl;
        if(both) {
        ...
        // b.b 649 X 500
        // cout << endl << "B.B In FrameDrawer.cc, Viewer.Run(), toShow image width: " << toShow.cols << ", height: " << toShow.rows << endl;
